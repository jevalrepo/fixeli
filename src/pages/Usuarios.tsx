...
export default function Usuarios() {
  ...
  async function handleEliminar(doctor: Perfil) {
    if (!confirm(`¿Eliminar a ${doctor.nombre_completo}?`)) return
    try {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-4 lg:space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg lg:text-xl font-bold text-slate-800">Usuarios</h1>
          <p className="text-sm text-gray-400 mt-0.5">Gestión del equipo médico</p>
        </div>
        <button
          onClick={abrirCrear}
          className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white px-3 lg:px-4 py-2 rounded-xl text-sm font-medium transition-colors"
        >
              ...
              {filtrados.map(p => (
                <div key={p.id} className="flex items-center gap-3 px-4 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-800 truncate">{p.nombre_completo}</p>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      className="p-2 rounded-lg text-gray-400 hover:bg-indigo-50 hover:text-indigo-500 transition-colors"
                    >
                      <Pencil size={14} />
                    </button>
                    <button
